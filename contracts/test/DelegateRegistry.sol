...
import {ISnapshotDelegation} from "../incentives/interfaces/ISnapshotDelegation.sol";

contract DelegateRegistry is ISnapshotDelegation {
    // The first key is the delegator and the second key a id.
    // The value is the address of the delegate
    mapping(address => mapping(bytes32 => address)) public override delegation;
    ...
    function setDelegate(bytes32 id, address delegate) public override {
    ...
    ///      The combination of msg.sender and the id can be seen as a unique key.
    /// @param id Id for which the delegate should be set
    function clearDelegate(bytes32 id) public override {
        address currentDelegate = delegation[msg.sender][id];
